...
var userProgress = mysqlTable("user_progress", {
  id: int("id").autoincrement().primaryKey(),
...
async function getAllActions() {
  ...
  return await db2.select().from(actions).orderBy(actions.id);
}
async function seedActions(actionsList) {
  ...
  const existing = await db2.select().from(actions).limit(1);
  if (existing.length > 0) {
    return;
  }
  await db2.insert(actions).values(actionsList);
}
async function getUserProgress(userId) {
  ...
  return await db2.select().from(userProgress).where(eq(userProgress.userId, userId));
}
...
async function getUserProgressStats(userId) {
  ...
  const [allActions, userProgressData] = await Promise.all([
    db2.select().from(actions),
    db2.select().from(userProgress).where(eq(userProgress.userId, userId))
  ]);
...
var db = drizzle2(process.env.DATABASE_URL);
var DEFAULT_OPENID = "student-default-openid";
async function createContext(opts) {
  let user = null;
  try {
    user = await sdk.authenticateRequest(opts.req);
  } catch (error) {
    const defaultUsers = await db.select().from(users).where(eq2(users.openId, DEFAULT_OPENID));
